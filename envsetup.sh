...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^batik_") ; then
        BATIK_BUILD=$(echo -n $1 | sed -e 's/^batik_//g')
    else
        BATIK_BUILD=
    fi
    export BATIK_BUILD
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the Batik GitHub
        T=$(gettop)
        cd $T > /dev/null
